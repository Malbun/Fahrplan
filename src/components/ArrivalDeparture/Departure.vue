<template>
  <div id="root">
    <button @click="run" class="bg-blue-300 text-black p-1.5 rounded-xl cursor-pointer hover:bg-gray-900 hover:text-white duration-300" id="departureSearchButton">Suchen</button>
...
function processStopEvent(stopEventPassed) {
  ...
  const estimated = stopEvent["ThisCall"]["CallAtStop"]["ServiceDeparture"]["EstimatedTime"];
  result.timetabled = getTimeAsString(new Date(stopEvent["ThisCall"]["CallAtStop"]["ServiceDeparture"]["TimetabledTime"]), false);
