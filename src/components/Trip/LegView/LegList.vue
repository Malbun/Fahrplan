...
function update() {
  ...
  for (const leg of props.trip.legs) {
    if (leg["type"] === "timed") {
      const newTimedLegView = h(TimedLegView, {leg: leg, id: `${props.trip.id}LegView${legCounter}`}, null);
      displayLegs.push(newTimedLegView);
    } else {
      const newTransferLegView = h(TransferLegView, {leg: leg, id: `${props.trip.id}LegView${legCounter}`},  null);
...
<template>
<div class="bg-gray-500 p-2 rounded-xl" :id="legContainerId">
