...
function renderInformation() {
  ...
  if (props.leg.ptMode !== "rail") { // checks if the leg mod (transport vehicle type) isn't a train
  ...
  if (tempQuay1.includes("$!")) { // checks if an estimated quay exists
    if (!tempQuay1.includes("/")) { // checks if the estimated quay has to be displayed as a quay change (ref. ZMUS)
  ...
  if (tempQuay1 !== "" && /\d/.test(tempQuay1)) { // checks if the temp variable isn't empty and contains a digit
    originQuay.value = `Gleis: ${tempQuay1}`; // set quay text to Gleis: tempQuay
  } else if (tempQuay1 !== "" && !(/\d/.test(tempQuay1))) { // checks if the temp variable isn't empty and does not contain any digits
  ...
  if (tempQuay2.includes("$!")) { // checks if an estimated quay exists
    if (!tempQuay2.includes("/")) { // checks if the estimated quay has to be displayed as a quay change (ref. ZMUS)
  ...
  if (tempQuay2 !== "" && /\d/.test(tempQuay2)) { // checks if the temp variable isn't empty and contains a digit
    destinationQuay.value = `Gleis: ${tempQuay2}`; // set quay text to Gleis: tempQuay
  } else if (tempQuay2 !== "" && !(/\d/.test(tempQuay2))) { // checks if the temp variable isn't empty and does not contain any digits
...
<template>
<div class="flex flex-row justify-between mb-1 mt-1 bg-gray-600 p-1.5 rounded-xl text-white">
  <div class="flex flex-row">
    <div class="flex flex-col justify-between">
      <div class="font-bold">{{getTimeAsString(new Date(props.leg.startTime))}}</div>
      <div>{{duration}}</div>
      <div>{{getTimeAsString(new Date(props.leg.endTime))}}</div>
    </div>
    <div class="ml-1 mr-1">
      <svg width="20" height="80">
        <line x1="10" y1="10" x2="10" y2="72" style="stroke: black; stroke-width: 4px"/>
