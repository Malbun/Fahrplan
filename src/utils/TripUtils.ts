...
export const getDurationFromString = (duration: string): number => {
  ...
  if (current.includes('H')) { // check if the string has a H for hours
  ...
  if (current.includes('M')) { // check if the string has an M for minutes
  ...
  if (current.includes('S')) { // check if the string has an S for seconds
      const splitByS: string[] = current.split('S'); // split the string by S
      if (parseInt(splitByS[0]) >= 30) { // check if the seconds have to be round up or down
